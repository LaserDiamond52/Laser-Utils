...
public abstract class LULootTables<T extends LUDataGenerator<T>> {
    ...
    {
        return new LootTableProvider(this.packOutput, Set.of(), this.lootTables(this.dataGenerator));
    }
    ...
    public static class BlockLoot<T extends LUDataGenerator<T>> extends BlockLootSubProvider implements AssetGenerator<Block>
    {
         ...
         * @param dataGenerator The {@link LUDataGenerator} type, specified by {@link T}
         */
        protected BlockLoot(T dataGenerator) {
        ...
        {
            return createSilkTouchDispatchTable(block, this.applyExplosionDecay(block, LootItem.lootTableItem(oreDrop).apply(SetItemCountFunction.setCount(UniformGenerator.between(min, max))).apply(ApplyBonusCount.addOreBonusCount(Enchantments.BLOCK_FORTUNE))));
        }
    ...
    public static class EntityLoot<T extends LUDataGenerator<T>> extends EntityLootSubProvider implements AssetGenerator<EntityType<?>>
    {
         ...
         * @param dataGenerator The {@link LUDataGenerator} type, specified by {@link T}
         */
        protected EntityLoot(T dataGenerator) {
            super(FeatureFlags.REGISTRY.allFlags());
            this.dataGenerator = dataGenerator;
        }

        /**
         * Creates a loot table drop
         ...
         */
        protected LootTable.Builder createLootTableDrop(Item item, float rolls, float minCount, float maxCount, float lootingMin, float lootingMax)
        {
//            return LootPool.lootPool().setRolls(ConstantValue.exactly(rolls)).add(LootItem.lootTableItem(item).apply(SetItemCountFunction.setCount(UniformGenerator.between(minCount, maxCount))).apply()).when(LootItemKilledByPlayerCondition.killedByPlayer());
            return LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(rolls)).add(LootItem.lootTableItem(item).apply(SetItemCountFunction.setCount(UniformGenerator.between(minCount, maxCount))).apply(LootingEnchantFunction.lootingMultiplier(UniformGenerator.between(lootingMin, lootingMax)))).when(LootItemKilledByPlayerCondition.killedByPlayer()));
        }
